...
describe("RateLimitService", () => {
  ...
  it("should allow request when under rate limit", async () => {
    // Mock DB response showing 2 requests in the window
  ...
  it("should block request when rate limit exceeded", async () => {
    const now = new Date();
    const oldestRequest = new Date(now.getTime() - 4 * 60 * 1000); // 4 minutes ago

    // First call returns count of 3
    ...
    // Second call returns oldest request timestamp
  ...
  it("should calculate retry-after correctly", async () => {
    const now = new Date();
    const oldestRequest = new Date(now.getTime() - 2 * 60 * 1000); // 2 minutes ago
    ...
    expect(result.isLimited).toBe(true);
    // Should be able to retry in ~3 minutes (5 min window - 2 min elapsed)
